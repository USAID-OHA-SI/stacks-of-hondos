...
#join datasets together 
    ...
    df_ue<-df_ue%>%     group_by(countryname,fundingagency,mech_code, mech_name, primepartner,)%>%
      ...
      dplyr::mutate(primepartner = dplyr::case_when(primepartner    == "FHI Development 360 LLC"    ~"FHI360",
                                                    ...
                                                    TRUE ~primepartner))%>%
      mutate("country_mech"=glue("{countryname}-{mech_code}"))%>%
...
# gt function ============================================================================
  ...
  #   
    get_ue_partner<-function(df, partner="country_mech"){
    df<-df_ue%>%
      select(-c("mech_code","fundingagency"))%>%
      filter(country_mech%in% partner)%>%
      gt()%>%
      cols_hide(
        columns=c("primepartner","countryname","country_mech",
                 ))%>%
...
# testing ============================================================================
    table_out<-"GitHub/stacks-of-hondos/Images/Partner tables/UE/redo"
    #to run for one OU testing below
     get_ue_partner(df_ue, "Zimbabwe-70473")%>%
