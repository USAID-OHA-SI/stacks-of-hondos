...
get_ou_agency_reg<-function(df){
  df<-df%>%
    ...
    dplyr::select (c(countryname,agency_category,fiscal_year,cop_budget_total,expenditure_amt))%>%
    ...
    tab_source_note(
      ...
      tab_header(
        title = glue::glue("COP19 & COP20 Program Financial Summary: Regional"),
...
get_ou_agency_reg(df_fsd)%>%
  gtsave(., path=table_out, filename="regional performance_all agencies.png")
